...
\begin{figure}
    ...
    \caption{Ukázka z~Node-RED -- vstupy a výstupy jednotlivých uzlů spojené pro vzájemnou komunikaci. V levé části
    se nachází soustava bloků typu \uv{inject}, pomocí kterých lze manuálně odeslat zprávu do vlastního bloku
    ovládající periferii červené LED diody. Pravá spodní část patří měření teploty pomocí bloku s DHT senzorem, který
    data předává do grafu a na blok reprezentující LED displej.}
\end{figure}
...
připojení k~brokeru MQTT není nutné zadávat do každého bloku zvlášť, ale jsou sdílené pro jeden konfigurační blok,
který je následně přiřazen ke každému bloku vyžadující broker MQTT.
...
\todo{něco o~dashboardu a možnostech?}
