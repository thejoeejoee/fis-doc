...
zrealizuji\todo{weird} vlastní rozšíření nástroje Node-RED -- předmětem budou detaily ze základní infrastruktury bloků a následně příklady konkrétních aplikací přímo přímo nasaditelných.
...
Ten se skládá ze dvou povinných částí a jedné nepovinné -- z~povinných částí jedná se o~programovou definici pro editor sítě v~jazyce Javascript a o~samotnou definici konfiguračního formuláře popsanou v~jazyce HTML.
...
\begin{code}[
    ...
    Důležitá je příprava části kanálů, které jsou náledně použity pro publikování a odběr zpráv -- v atributu
...
Třída \ic{FisNode} nabízí díky své implementace další vlastnosti\todo{weird}:
\begin{itemize}
    ...
    události typu \ic{\'close\'} nad konkrétním blokem, čímž získá informaci od nástroje Node-RED, že došlo k
    ...
    Práce s frontou typu \textit{FIFO} je umístěna v metodě \ic{_publish} a pro jednotlivé bloky je tato fronta zcela
    transparentní.
\end{itemize}
...
\begin{figure}
    ...
    stav, kdy je uzel připojen, avšak aplikace nezasílá či nepříjmá data (druhý případ na obrázku) a poslední stav,
...
nástroje Node-RED \mbox{\ic{RED.nodes.getNode}} lze získat instanci bloku, která reprezentuje IoT uzel připojený přes
MQTT kanály.
...
Druhé volání je použití již popsané metody \ic{appSubscribe}, blok si zde registruje odběr na data přicházející
z~uzlu.
...
\begin{code}[
    ...
    senzory typu DHT měřící teplotu a vlhkost okolí. Konstruktor této třídy je zodpovědný za konfiguraci aplikace na
    uzlu za pomoci metody \ic{config} a následnou registrace odběru zprávy z uzlu -- funkce volaná na příchozí zprávy
    z nich přejímá data a zasílá je dále do sítě nástroje Node-RED (volání \ic{this.send})}.
]
class DhtSensor {
    constructor(config) {
        RED.nodes.createNode(this, config);
